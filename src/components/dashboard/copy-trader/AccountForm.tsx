...
const BROKERS: { value: BrokerType; label: string; servers?: string }[] = [
  {
    value: "mt4",
    label: "MetaTrader 4",
    servers: "Ej: IG-DEMO, FxPro.com-Real02",
  },
  {
    value: "mt5",
    label: "MetaTrader 5",
    servers: "Ej: ActivTrades-Server, Binary.com-Server",
  },
  { value: "ctrader", label: "cTrader", servers: "Escriba 'unknown'" },
  {
    value: "fxcm_fc",
    label: "FXCM",
    servers: "URL: http://www.fxcorporate.com/Hosts.jsp",
  },
...
export default function AccountForm({
  ...
  const handleSelectChange = (name: string, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
      [name]: ["type", "status", "alert_email"].includes(name)
        ? Number(value)
        : value,
    }));
  };
  ...
  const showEnvironment = ["fxcm_fc", "lmax", "ctrader", "tradovate"].includes(
    formData.broker || "",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === "success") {
        ...
        setMessage({
          type: "error",
          text: response.message || "Error al procesar la cuenta.",
        });
      }
    } catch (error: any) {
  ...
  const inputClasses =
    "bg-background/40 border-white/10 rounded-xl h-11 focus:ring-primary focus:border-primary transition-all";
  const labelClasses =
    ...
    <Card className="max-w-4xl mx-auto backdrop-blur-xl bg-card/60 border-white/5 shadow-2xl overflow-hidden rounded-[2rem]">
      <CardHeader className="p-8 pb-4 relative z-10">
        ...
        <CardDescription className="text-muted-foreground/80 font-medium max-w-lg mt-2">
          Conecta tus cuentas de inversión mediante nuestra API segura para la
          operativa de copiado institucional.
        </CardDescription>
      </CardHeader>
