...
const timeframeOrder: HistoryRequest["timeframe"][] = ["M1", "M5", "M15", "M30", "H1", "H4", "D1", "W1", "MN1"];
...
export function TradingChart() {
  ...
  const [timeframe, setTimeframe] = useState<HistoryRequest["timeframe"]>("M15");
  ...
  useEffect(() => {
    ...
    const gradient = ctx.createLinearGradient(0, 0, 0, canvasRef.current.height || 300);
  ...
  return (
    <div className="rounded-2xl border border-[#2a3050] bg-[#11152a] text-white shadow-xl">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="relative flex-1 min-w-[240px]" ref={dropdownRef}>
              <p className="text-xs uppercase tracking-wide text-blue-300">Par seleccionado</p>
              <button
                type="button"
                onClick={() => setDropdownOpen((open) => !open)}
                className="flex w-full items-center justify-between rounded-2xl border border-blue-500/30 bg-[#050816] px-4 py-2 text-left text-lg font-semibold text-white shadow-[inset_0_0_10px_rgba(15,23,42,0.6)] transition hover:border-blue-400 focus-visible:border-blue-500 focus-visible:outline-none"
              >
              ...
              {isDropdownOpen ? (
                <div className="absolute z-30 mt-2 w-full rounded-2xl border border-[#1e2650] bg-[#050816]/95 p-3 shadow-2xl backdrop-blur">
                  <div className="mb-3 flex items-center gap-2 rounded-2xl border border-[#1e2650] bg-[#0d1328] px-3 py-2 text-sm text-slate-300">
                          ...
                          <button
                            ...
                            className={`flex w-full flex-col rounded-xl border px-3 py-2 text-left text-sm transition ${
                              active
                                ...
                                : "border-transparent bg-[#0d1328] text-slate-200 hover:border-blue-500/30"
                            }`}
                          >
                            <span className="font-semibold">{instrument.symbol}</span>
                            <span className="text-xs text-slate-400">{instrument.description}</span>
                          </button>
                        );
                      })
                    ) : (
                      <p className="py-10 text-center text-xs text-slate-500">No se encontraron símbolos.</p>
                    )}
                  </div>
                </div>
              ) : null}
          ...
          <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs transition ${
                  timeframe === key
                    ...
                    : "border border-transparent bg-[#0d1328] text-slate-400 hover:border-[#1d2750]"
                }`}
              >
            ...
            <button
              onClick={() => refetch()}
              disabled={isLoading}
              className="flex items-center gap-2 rounded-full border border-[#2a3050] bg-[#0d1328] px-3 py-1 text-xs text-slate-300 transition hover:border-blue-500 disabled:opacity-50"
            >
              <RefreshCcw size={14} className={isLoading ? "animate-spin text-blue-400" : ""} />
              Sync
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-6 px-6 py-4 text-sm">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">Último</p>
          <p className="text-2xl font-semibold text-white">{lastPrice.toFixed(5)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">Variación</p>
          <p className={`text-2xl font-semibold ${priceChange >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
            {priceChange >= 0 ? "+" : ""}
        ...
        {detail ? (
          <>
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">Volumen mín.</p>
              <p className="text-2xl font-semibold text-white">{detail.volume_min}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-500">Volumen máx.</p>
              <p className="text-2xl font-semibold text-white">{detail.volume_max}</p>
            </div>
          </>
        ) : null}
      </div>

      <div className="h-80 rounded-2xl border border-[#1a2242] bg-[#0a0e1a] p-4">
        {error ? (
          ...
          <div className="flex h-full items-center justify-center text-sm text-slate-400">Cargando gráfico…</div>
        ) : candles.length ? (
